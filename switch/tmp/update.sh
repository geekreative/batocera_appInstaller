         ...
         ####################################################################
            u=/userdata/system/switch/extra/batocera-switch-desktopupdater.sh
               rm "$u" 2>/dev/null
                  ...
                  echo "  wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O /tmp/.batocera-switch-updater.sh https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-updater.sh" >> "$u"
                  ##echo "  curl -sSf https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-updater.sh -o /tmp/.batocera-switch-updater.sh " >> "$u"
...
EMULATORS="$(echo $EMULATORS | sed 's/ /-/g')"
   ...
   if [[ ! -f $cfg ]]; then 
      link_defaultconfig=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-config.txt
...
##
T=$THEME_COLOR_YUZU
killall -9 yuzu yuzuea yuzu-room yuzu.AppImage yuzuEA.AppImage 2>/dev/null
if [ "$N" = "1" ]; then C=""; else C="$E/$N"; fi
   function generate_launcher() {
         ...
         echo 'ln -sf /userdata/system/switch/yuzu.AppImage /usr/bin/yuzu 2>/dev/null' >> "$f"
         ...
         echo 'ulimit -H -n 819200 && ulimit -S -n 819200 && sysctl -w fs.inotify.max_user_watches=8192000 vm.max_map_count=2147483642 fs.file-max=8192000 >/dev/null 2>&1' >> "$f"
         echo 'rom="$(echo "$@" | sed '\''s,-f -g ,,g'\'')" ' >> "$f"
         echo 'if [[ "$rom" = "" ]]; then ' >> "$f"
         echo '  LC_ALL=C NO_AT_BRIDGE=1 QT_FONT_DPI=96 QT_SCALE_FACTOR=1 GDK_SCALE=1 LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" QT_PLUGIN_PATH=/usr/lib/qt/plugins:/userdata/system/switch/extra/lib/qt5plugins:/usr/plugins:${QT_PLUGIN_PATH} QT_QPA_PLATFORM_PLUGIN_PATH=${QT_PLUGIN_PATH} XDG_CONFIG_HOME=/userdata/system/configs XDG_CACHE_HOME=/userdata/system/.cache QT_QPA_PLATFORM=xcb /userdata/system/switch/_yuzu.AppImage -f -g > >(tee "$log1") 2> >(tee "$log2" >&2) ' >> "$f" 
         echo 'else ' >> "$f"
         ...
         echo '    QT_XKB_CONFIG_ROOT=/usr/share/X11/xkb LC_ALL=C.utf8 NO_AT_BRIDGE=1 XDG_MENU_PREFIX=batocera- XDG_CONFIG_DIRS=/etc/xdg XDG_CURRENT_DESKTOP=XFCE DESKTOP_SESSION=XFCE QT_FONT_DPI=96 QT_SCALE_FACTOR=1 GDK_SCALE=1 LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" GDK_SCALE=1 QT_PLUGIN_PATH=/usr/lib/qt/plugins:/userdata/system/switch/extra/lib/qt5plugins:/usr/plugins:${QT_PLUGIN_PATH} QT_QPA_PLATFORM_PLUGIN_PATH=${QT_PLUGIN_PATH} XDG_CONFIG_HOME=/userdata/system/configs XDG_CACHE_HOME=/userdata/system/.cache QT_QPA_PLATFORM=xcb /userdata/system/switch/_yuzu.AppImage -f -g "$ROM" 1>"$log1" 2>"$log2" ' >> "$f"
         echo '  else ' >> "$f"
         echo '    ROM="$rom" ' >> "$f"
         echo '    QT_XKB_CONFIG_ROOT=/usr/share/X11/xkb LC_ALL=C.utf8 NO_AT_BRIDGE=1 XDG_MENU_PREFIX=batocera- XDG_CONFIG_DIRS=/etc/xdg XDG_CURRENT_DESKTOP=XFCE DESKTOP_SESSION=XFCE QT_FONT_DPI=96 QT_SCALE_FACTOR=1 GDK_SCALE=1 LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" GDK_SCALE=1 QT_PLUGIN_PATH=/usr/lib/qt/plugins:/userdata/system/switch/extra/lib/qt5plugins:/usr/plugins:${QT_PLUGIN_PATH} QT_QPA_PLATFORM_PLUGIN_PATH=${QT_PLUGIN_PATH} XDG_CONFIG_HOME=/userdata/system/configs XDG_CACHE_HOME=/userdata/system/.cache QT_QPA_PLATFORM=xcb /userdata/system/switch/_yuzu.AppImage -f -g "$ROM" 1>"$log1" 2>"$log2" ' >> "$f"
         echo '  fi ' >> "$f"
         echo 'fi' >> "$f"
         dos2unix "$f" 2>/dev/null && chmod 777 "$f" 2>/dev/null
   }
   f=/userdata/system/switch/extra/yuzu/yuzu
   if [[ -e $f ]]; then
      f=/userdata/system/switch/yuzu.AppImage
      if [[ -e $f ]]; then
         size=$(wc -c $f | awk '{print $1}')
         if [[ "$size" -lt 1000000 ]]; then
            if [[ "$(cat $f | grep '_yuzu.AppImage')" != "" ]]; then
             f=/userdata/system/switch/_yuzu.AppImage 
               if [[ -e $f ]]; then
                  size=$(wc -c $f | awk '{print $1}')
                  if [[ "$size" -gt 5000000 ]]; then
                   sleep 0.4444
                   echo -e "${T}██ $C   ${T}YUZU   [${W}OK${T}]   found yuzu.Appimage in /userdata/system/switch/"
                   sleep 0.4444
                   ...
                   echo
                     ...
                     sleep 0.4444
                     echo -e "${T}██ $C   ${T}YUZU   [${W}!!${T}]   place custom yuzu.Appimage in /userdata/system/switch/"
                     sleep 0.4444
                     echo
                  fi
               else 
               ...
               if [[ "$(cat $f | grep '/extra/yuzu/yuzu')" != "" ]]; then
                sleep 0.4444
                echo -e "${T}██ $C   ${T}YUZU   [${W}OK${T}]   found previous yuzu, skipping update"
                sleep 0.4444
                echo -e "${T}■■       ${T}       ${T}       ${W}❯${T} generating launcher"
                sleep 0.4444
                ...
                echo
                  generate_launcher
                  sed -i 's,/userdata/system/switch/_yuzu.AppImage,/userdata/system/switch/extra/yuzu/yuzu,g' /userdata/system/switch/yuzu.AppImage 2>/dev/null
               fi
            fi
         else 
         ...
         sleep 0.4444
         echo -e "${T}██ $C   ${T}YUZU   [${W}OK${T}]   found previous yuzu"
         sleep 0.4444
         echo -e "${T}■■       ${T}       ${T}       ${W}❯${T} generating launcher"
         sleep 0.4444
         echo -e "${T}##       ${T}       ${T}       ${T}❯${T} or replace /userdata/system/switch/yuzu.AppImage"
...
if [ "$N" = "1" ]; then C=""; else C="$E/$N"; fi
   function generate_launcher() {
         ...
         echo 'mkdir -p /userdata/system/configs/Ryujinx/system 2>/dev/null; cp -rL /userdata/bios/switch/*.keys /userdata/system/configs/Ryujinx/system/ 2>/dev/null ' >> "$f"
         echo 'rm /usr/bin/yuzu 2>/dev/null; rm /usr/bin/yuzu-room 2>/dev/null' >> "$f"
         echo 'ln -sf /userdata/system/switch/yuzuEA.AppImage /usr/bin/yuzu 2>/dev/null' >> "$f"
         echo 'mkdir -p /userdata/system/switch/logs 2>/dev/null ' >> "$f"
         ...
         echo 'rom="$(echo "$@" | sed '\''s,-f -g ,,g'\'')" ' >> "$f"
         echo 'if [[ "$rom" = "" ]]; then ' >> "$f"
         echo '  LC_ALL=C NO_AT_BRIDGE=1 QT_FONT_DPI=96 QT_SCALE_FACTOR=1 GDK_SCALE=1 LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" QT_PLUGIN_PATH=/usr/lib/qt/plugins:/userdata/system/switch/extra/lib/qt5plugins:/usr/plugins:${QT_PLUGIN_PATH} QT_QPA_PLATFORM_PLUGIN_PATH=${QT_PLUGIN_PATH} XDG_CONFIG_HOME=/userdata/system/configs XDG_CACHE_HOME=/userdata/system/.cache QT_QPA_PLATFORM=xcb /userdata/system/switch/_yuzuEA.AppImage -f -g > >(tee "$log1") 2> >(tee "$log2" >&2) ' >> "$f" 
         echo 'else ' >> "$f"
         echo '  rm /tmp/switchromname 2>/dev/null ' >> "$f" 
         echo '    echo "$rom" >> /tmp/switchromname 2>/dev/null ' >> "$f" 
         echo '      /userdata/system/switch/extra/batocera-switch-nsz-converter.sh ' >> "$f" 
         ...
         echo '  if [[ "$fs" == *"EXT"* ]] || [[ "$fs" == *"BTR"* ]]; then ' >> "$f"
         echo '    rm /tmp/yuzurom 2>/dev/null; ln -sf "$rom" "/tmp/yuzurom"; ROM="/tmp/yuzurom"; ' >> "$f"
         echo '    QT_XKB_CONFIG_ROOT=/usr/share/X11/xkb LC_ALL=C.utf8 NO_AT_BRIDGE=1 XDG_MENU_PREFIX=batocera- XDG_CONFIG_DIRS=/etc/xdg XDG_CURRENT_DESKTOP=XFCE DESKTOP_SESSION=XFCE QT_FONT_DPI=96 QT_SCALE_FACTOR=1 GDK_SCALE=1 LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" GDK_SCALE=1 QT_PLUGIN_PATH=/usr/lib/qt/plugins:/userdata/system/switch/extra/lib/qt5plugins:/usr/plugins:${QT_PLUGIN_PATH} QT_QPA_PLATFORM_PLUGIN_PATH=${QT_PLUGIN_PATH} XDG_CONFIG_HOME=/userdata/system/configs XDG_CACHE_HOME=/userdata/system/.cache QT_QPA_PLATFORM=xcb /userdata/system/switch/_yuzuEA.AppImage -f -g "$ROM" 1>"$log1" 2>"$log2" ' >> "$f"
         echo '  else ' >> "$f"
         echo '    ROM="$rom" ' >> "$f"
         echo '    QT_XKB_CONFIG_ROOT=/usr/share/X11/xkb LC_ALL=C.utf8 NO_AT_BRIDGE=1 XDG_MENU_PREFIX=batocera- XDG_CONFIG_DIRS=/etc/xdg XDG_CURRENT_DESKTOP=XFCE DESKTOP_SESSION=XFCE QT_FONT_DPI=96 QT_SCALE_FACTOR=1 GDK_SCALE=1 LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" GDK_SCALE=1 QT_PLUGIN_PATH=/usr/lib/qt/plugins:/userdata/system/switch/extra/lib/qt5plugins:/usr/plugins:${QT_PLUGIN_PATH} QT_QPA_PLATFORM_PLUGIN_PATH=${QT_PLUGIN_PATH} XDG_CONFIG_HOME=/userdata/system/configs XDG_CACHE_HOME=/userdata/system/.cache QT_QPA_PLATFORM=xcb /userdata/system/switch/_yuzuEA.AppImage -f -g "$ROM" 1>"$log1" 2>"$log2" ' >> "$f"
         echo '  fi ' >> "$f"
   ...
   f=/userdata/system/switch/extra/yuzuea/yuzu
   if [[ -e $f ]]; then
      f=/userdata/system/switch/yuzuEA.AppImage
      if [[ -e $f ]]; then
         size=$(wc -c $f | awk '{print $1}')
         if [[ "$size" -lt 1000000 ]]; then
            if [[ "$(cat $f | grep '_yuzuEA.AppImage')" != "" ]]; then
             f=/userdata/system/switch/_yuzuEA.AppImage 
               if [[ -e $f ]]; then
                  size=$(wc -c $f | awk '{print $1}')
                  if [[ "$size" -gt 5000000 ]]; then
                   sleep 0.4444
                   echo -e "${T}██ $C   ${T}YUZUEA   [${W}OK${T}]   found yuzuEA.Appimage in /userdata/system/switch/"
                   sleep 0.4444
                   echo -e "${T}■■       ${T}         ${T}       ${W}❯${T} generating launcher"
                   sleep 0.4444
                   echo
                     ...
                     sleep 0.4444
                     echo -e "${T}██ $C   ${T}YUZUEA   [${W}!!${T}]   place custom yuzuEA.Appimage in /userdata/system/switch/"
                     sleep 0.4444
                     echo
                  fi
               else 
               ...
               if [[ "$(cat $f | grep '/extra/yuzuea/yuzu')" != "" ]]; then
                sleep 0.4444
                echo -e "${T}██ $C   ${T}YUZUEA   [${W}OK${T}]   found previous yuzuEA, skipping update"
                sleep 0.4444
                echo -e "${T}■■       ${T}         ${T}       ${W}❯${T} generating launcher"
                sleep 0.4444
                ...
                echo
                  generate_launcher
                  sed -i 's,/userdata/system/switch/_yuzuEA.AppImage,/userdata/system/switch/extra/yuzuea/yuzu,g' /userdata/system/switch/yuzuEA.AppImage 2>/dev/null
               fi
            fi
         else 
           if [[ "$size" -gt 50000000 ]]; then
         ...
         sleep 0.4444
         echo -e "${T}██ $C   ${T}YUZUEA   [${W}OK${T}]   found previous yuzuEA"
         sleep 0.4444
         echo -e "${T}■■       ${T}         ${T}       ${W}❯${T} generating launcher"
         sleep 0.4444
         echo -e "${T}##       ${T}         ${T}       ${T}❯${T} or replace /userdata/system/switch/yuzuEA.AppImage"
...
# ------------------------------------------------------------------- 
   extraurl="https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra"
# ------------------------------------------------------------------- 
...
#   wget -q --tries=10 --no-check-certificate --no-cache --no-cookies -O "/userdata/system/switch/configgen/mapping.csv" "https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/configgen/mapping.csv"
...
# -------------------------------------------------------------------
# prepare patcher 
url_patcher="https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-patcher.sh"
...
url_updater=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-updater.sh
url_portsupdater=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/roms/ports/Switch%20Updater.sh
url_portsupdaterkeys=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/roms/ports/Switch%20Updater.sh.keys   
...
# additional pass for folks who have issues connecting to github
   function get() {
      file="$1"
      path=/userdata/system/switch/configgen/sdl2
      url=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/configgen/sdl2
         mkdir -p $path 2>/dev/null
            if [[ ! -e "$path/$file" ]]; then
               cd $path
...
sdlurl=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-libSDL2.so
   if [[ ! -e "$sdl" ]]; then 
...
urldir="https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra"
...
url=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/translations
mkdir -p $path 2>/dev/null
...
# GET TRANSLATOR
translator=https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-translator.sh
...
# include display output: 
   ...
   mkdir /userdata/system/switch 2>/dev/null; mkdir /userdata/system/switch/extra 2>/dev/null
      if [[ ( -e "$tput" && "$(wc -c "$tput" | awk '{print $1}')" < "444" ) || ( ! -e "$tput" ) ]]; then
         ...
         ##curl -sSf "https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-tput" -o "/userdata/system/switch/extra/batocera-switch-tput"
      fi
      if [[ ( -e "$libtinfo" && "$(wc -c "$libtinfo" | awk '{print $1}')" < "444" ) || ( ! -e "$libtinfo" ) ]]; then
         ...
         ##curl -sSf "https://github.com/geekreative/batocera_switch_installer/raw/main/emulators/system/switch/extra/batocera-switch-libtinfo.so.6" -o "/userdata/system/switch/extra/batocera-switch-libtinfo.so.6"
      fi
